...
```{r nmds-plot, warning = FALSE, fig.height = 6}
nmds$points %>% broom::tidy() %>% rename(sample = .rownames) %>%
  inner_join(samples, by = 'sample') %>%
  ggplot(aes(x = MDS1, y = MDS2, shape = TransectName, colour = AverageSoilRelativeHumidity)) +
  geom_point() +
  geom_label(aes(label = SiteName), nudge_y = 0.1, colour = 'black') +
  scale_colour_viridis_c() +
  theme(
    legend.position = 'bottom'
  )
